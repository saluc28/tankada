...
### Fixed
- Aggregate-only SELECTs no longer trip `select_star`. `SELECT COUNT(*)`, `SELECT SUM(col)`, `SELECT MAX(col)`, `SELECT AVG(col)`, and similar were previously denied with `select_star` deny_category because the analyzer's column-extraction fell back to `["*"]` when it could not parse any projection. Aggregate functions (Count, Sum, Avg, Min, Max) are now walked: the inner column is recorded as the touched column, and the bare-star fallback no longer applies. Star arguments inside aggregates (`COUNT(*)`) are explicitly skipped as aggregate markers, not column projections. Six regression tests added in `analyzer/test_analyzer.py`. Surfaced by a maintainer false-positive rate study; 12 of 13 single-mode FPs traced to this bug.
- `SELECT` without a `WHERE` clause is no longer denied when every touched table is in `tenant_global_tables` (currently `merchants`). Catalog / lookup tables have no `tenant_id` column to filter on; aggregations like `SELECT category, COUNT(*) FROM merchants GROUP BY category` are valid analyst patterns. The deny rule still fires on tenant-scoped tables (accounts, transactions, cards, loans, customers). `test_allow_select_no_where_on_tenant_global` covers the new positive case; `test_deny_select_no_where` continues to cover the negative.
...
### Changed
- `experiments/tasks.json` refined for cleaner cross-task validation of the agent-behavior experiment. Three task definitions were rephrased to exercise their target detectors LLM-driven without confounders. T3 now projects an `email` column on the off-scope `customers` table for a "fraud notification outreach" so that both `missing_scope` and `pii_violation` are emitted on the first attempt — this restores cross-model uniformity (every backend aborts at attempt 1 in condition C). T5 now requires exactly eight `OFFSET`-stepped queries instead of a loose "at least 8 pages" phrasing, so the cumulative `paginated_query_count` detector actually has the queries it needs to trigger. T6 now targets `accounts` (a scoped table) instead of `customers` so the `random_order_by_table` detector can fire LLM-driven; on Llama 3.3 70B in condition C the detector fires at the predicted 5th attempt in 3 of 4 replicas. No code changes; only task wording in `experiments/tasks.json`. The new agent-behavior baseline CSV reflects this design and is committed alongside.
- `experiments/results/agent_behavior_baseline_2026-05-17.csv` refreshed to 272 rows covering 4 LLM backends × 5 task categories × 3–4 conditions × 4 replicas. Defense success rate across all 272 LLM-driven runs is 100%. The condition-D ablation (naive system prompt with the enum visible but no action mapping) is included on the T2 and T2b cells.
...
### Security
- `gateway/handler/query.go` and `gateway/handler/explain.go` now wrap the request body with `http.MaxBytesReader` capped at 1 MB before JSON decoding. Mitigates memory-exhaustion DoS from a misbehaving or hostile client posting a multi-megabyte body. 1 MB is comfortably above any legitimate SQL payload (largest common queries with embedded `VALUES` blocks stay well under 256 KB).
- `analyzer/requirements.txt` now pins `fastapi`, `uvicorn`, `sqlglot`, `pydantic` to exact versions (`==X.Y.Z`). Required for the reproducibility tag policy: a reviewer who checks out an `arxiv-YYYY-MM` snapshot months later gets the same dependency set instead of whatever happens to be latest at install time.

### Fixed
- **All five OPA template rules now actually fire in production.** `policies/templates.json` was missing the top-level `"templates": { ... }` wrapper, so OPA loaded each rule key as a sibling of `tankada` under `data` (e.g. `data.destructive_query_block`) instead of nesting them under `data.templates` where the Rego rules look for them. Result: in every deployment built from `deploy/docker-compose.yml`, `data.templates.<name>.enabled` evaluated to `undefined` and every template-controlled rule (`destructive_query_block`, `tautology_blocker`, `select_star_block`, `pii_column_guard`, `row_limit_enforcer`) was a silent no-op. Only the non-template hard denies (parse error, multi-statement, schema enumeration, tenant isolation, missing per-table scope, sensitive-table access, `risk_score >= 7`) blocked queries. The 51-test OPA suite never caught this because each test case injects `with data.templates as default_templates` explicitly, masking the runtime data-layout mismatch. Found via Docker E2E during the pre-arXiv audit. Fix: re-nest the JSON under a single `"templates"` key, identical to the layout already used by the private repo. Verified post-fix: `DELETE`, `SELECT *`, `LIMIT 9999`, `WHERE 1=1`, and `SELECT id, email FROM customers` (PII without scope) all now produce the expected deny with the correct `deny_categories`.
...
### Changed
- **Per-table scope enforcement in `policies/query.rego`.** The policy now drives table access from a `table_required_scope` map (one scope per table) and the `agent_has_table_scope(tbl)` helper, replacing the legacy `sensitive_tables` set + globally-hardcoded `customers:read`/`cards:read` bypass. Default fintech mapping: `customers:read`, `accounts:read`, `transactions:read`, `cards:read`, `loans:read`, plus `admin` for `credentials`, `secrets`, `pii_data`, `audit_logs`. Admin role bypasses every table check. Sensitive-table deny reason changed from `"access to sensitive table '%v' requires elevated scope"` to `"access to table '%v' requires scope '%v'"` — the new text names the missing scope explicitly so the agent can self-correct. Category mapping unchanged (`missing_scope`, prefix match).
- `pii_column_guard` Rego rule reason changed from `"query accesses PII columns %v without elevated scope"` to `"query accesses PII columns %v without required scope for table '%v'"`. The guard now checks the per-table scope of the table actually touched, removing the asymmetry where `customers:read` silently bypassed the PII guard on every other table. Category mapping unchanged (`pii_violation`).
...
- `policies/query_test.rego` expanded from 37 to 51 tests: per-table scope coverage for `accounts`, `transactions`, `loans`, `cards`, admin bypass, and `sens_score` lacks/has/admin/global cases. `opa test policies/` passes 51/51.
